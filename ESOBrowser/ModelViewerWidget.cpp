...
struct GrannyFileDeleter {
	inline void operator()(granny_file* file) const {
		GrannyFreeFile(file);
	}
};
using GrannyFile = std::unique_ptr<granny_file, GrannyFileDeleter>;

ModelViewerWidget::ModelViewerWidget(DataStorage* storage, QWidget* parent) : QWidget(parent), m_storage(storage) {
	ui.setupUi(this);
...
void ModelViewerWidget::loadSingleModel(uint64_t key) {
	try {
		auto data = m_storage->filesystem()->readFileByKey(key);
		...
		if (root.Type && root.Object) {
			m_structureModel->appendRow(objectToStructure(&root, m_storage->filesystemModel()->nameForId(key)));
		}
	}
